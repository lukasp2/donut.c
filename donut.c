...
int main() {
    ...
    double A, B = 0; // torus rotation around x and z

    Matrix* torus = matrix_init(1, 3);
    Matrix* normal = matrix_init(1, 3);

     // location of light source at (0, 1, -1)
    ...
    // z-buffer and framebuffer for rendering
    ...
    for (;;) {
        ...
        for (double u = 0; u < 2*M_PI; u += 0.04) {
            for (double v = 0; v < 2*M_PI; v += 0.07) {
		// 1. describe a circle
		...
		// if (u > 6) matrix_print(rotation, "rotation");
		...
		// 6. store a z value of what we draw
